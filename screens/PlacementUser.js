...
const PlacementUser = ({ navigation }) => {
    ...
    useEffect(() => {
        const fetchSeniors = async () => {
            try {
                ...
                const seniorsQuery = query(collection(db, 'seniors'), where('role', '==', 'placement_coordinator'));
    ...
    const renderItem = ({ item }) => (
        <TouchableOpacity 
            onPress={() => navigation.navigate("PlacementUserDetail", { uid: item.uid, userId: item.id })} // Pass the document ID
        >
